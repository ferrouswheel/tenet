...
- [ ] Create `android/tenet-ffi/` crate with minimal UDL covering identity init, sync, list/send messages
- [ ] Verify `cargo-ndk` build succeeds for all three ABIs
- [ ] Generate Kotlin bindings and verify JNI linkage in a stub Android project
- [ ] Implement `TenetClient` Rust struct (wraps `RelayClient` + `Storage` behind a `Mutex`)
- [ ] Basic Android project skeleton: `MainActivity`, Hilt setup, `TenetRepository`
- [ ] Timeline screen (read-only): fetch and display public messages
